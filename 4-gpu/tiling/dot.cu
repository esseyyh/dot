...
__global__ void matvec_kernel_tiled(float *A, float *B, float *C, int M,
                                    int K) {
  ...
  for (int m = 0; m < (K + blockDim.x - 1) / blockDim.x; ++m) {
    if (row < M && m * blockDim.x + threadIdx.x < K) {
      tile[threadIdx.x] = A[row * K + m * blockDim.x + threadIdx.x];
    } else {
...
void matvec(float *A, float *B, float *C, int M, int K) {
  ...
  matvec_kernel_tiled<<<grid_size, block_size, shared_mem_size>>>(d_A, d_B, d_C,
                                                                  M, K);
